...
def get_user_for_process_id(process_id):
    return (
        system_call('ps -o user= -p ' + str(process_id)).strip().decode('utf-8')
    )
...
def plot_gpu_usage():
    ...
    for n_r in range(num_rows):
        for n_c in range(num_cols):
            ...
            axs[n_c, n_r].pie(x=usages, autopct='%.1f%%', explode=explodes, labels=users, pctdistance=0.5)  # noqa: E501
...
def main():
    ...
    parser.add_argument('-user', '--user', type=str, default='prasse')
